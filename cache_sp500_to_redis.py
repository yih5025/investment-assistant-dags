...
def sp500_caching_dag():
    ...
    def fetch_sp500_current_data():
        ...
        # 시장 개장 중: 매 실행마다
        # 시장 마감 중: 정각(00분)에만 (1시간마다)
        if not market_open and current_minute != 0:
            logger.info(f"🔒 시장 마감 중 - 정각이 아님 ({current_minute}분). 작업 건너뜀.")
            return []
        
        if market_open:
            logger.info("📊 SP500 현재 데이터 조회 시작 (시장 개장 중 - 1분마다)")
        else:
            logger.info("📊 SP500 현재 데이터 조회 시작 (시장 마감 중 - 1시간마다)")
